		...
		({ target }) => {
			...
			for (const file of csharpFiles) {
				...
				content = content.replace(
					new RegExp('Owner = "NVGUQ1qyL4SdSm7sVmGVkXetjEsvw2L3NT";', 'g'),
					`Owner = "${config.initialOwner}";`,
				);
				content = content.replace(
					new RegExp('FLMHash = "5b53998b399d10cd25727269e865acc785ef5c1a";', 'g'),
					`FLMHash = "${config.flmHash}";`,
				);
				content = content.replace(
					new RegExp('Factory = "52bf47559436d3572a9b1cb83c056dc39cb42d0d";', 'g'),
					`Factory = "${config.swapFactoryHash}";`,
				);
