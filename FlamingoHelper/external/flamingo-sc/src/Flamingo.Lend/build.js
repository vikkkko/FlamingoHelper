		...
		({ target }) => {
			...
			for (const file of csharpFiles) {
				...
				content = content.replace(
					new RegExp('INITIAL_OWNER = "14131211100f0e0d0c0b0a090807060504030201";', 'g'),
					`INITIAL_OWNER = "${config.initialOwner}";`,
				);
				if (config.ADD_DANGEROUS_METHODS) {
					content = content.replace(new RegExp('// \\[TESTNET ONLY\\] ?', 'g'), '');
				}
