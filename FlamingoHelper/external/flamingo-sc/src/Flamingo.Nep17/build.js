		...
		({ target }) => {
			...
			for (const file of csharpFiles) {
				...
				for (const tokenConfig of config.tokens) {
					...
					content = content.replace(new RegExp('class Nep17Token', 'g'), `class ${newContractClassName}`);
					// Update each mapping
					content = content.replace(new RegExp('Name\\(\\) => "Test Token"', 'g'), `Name() => "${tokenConfig.tokenName}"`);
					content = content.replace(new RegExp('Symbol\\(\\) => "TEST"', 'g'), `Symbol() => "${tokenConfig.tokenSymbol}"`);
					content = content.replace(new RegExp('Decimals\\(\\) => 8', 'g'), `Decimals() => ${tokenConfig.tokenDecimals}`);
					content = content.replace(
						new RegExp('InitialValue\\("NaBUWGCLWFZTGK4V9f4pecuXmEijtGXMNX"', 'g'),
						`InitialValue("${config.initialOwner}"`,
					);
